...
const Project = () => {
  ...
  return (
    <div className='sm:w-full w-auto h-auto sm:px-15 px-8 py-10'>
      ...
      <div className='mt-10 w-full  flex flex-wrap px-10 gap-10 line-clamp-1'>
        {data.length > 0 ? (
          data.map((project) => (
            <div
              onClick={() => handlebutton(project._id, project.name)}
              key={project._id}
              className="sm:w-[30%] w-[15rem] h-auto bg-gradient-to-tr from-[#CB7700] to-[#FFD68D]  text-white border-[1px] border-[#DCD6D6] px-5 pb-5 py-7 rounded-2xl  cursor-pointer hover:shadow-[-10px_10px_20px_rgba(0,0,0,0.10)] transition-transform transform duration-300"
            >
              <div className="w-full sm:h-[25vh] h-[20vh] rounded-2xl overflow-hidden rounded-md">
                <img
                  src={project.image}
                  alt={project.name}
              ...
              <div className='h-[8rem] pb-4'>
                <h1 className="sm:text-md text-sm font-serifpro font-bold sm:mt-5 mt-3">
                  {project.name}
                </h1>
